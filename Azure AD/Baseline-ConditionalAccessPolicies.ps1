...
.SYNOPSIS
    ...
    4. GRANT - Require MAM approved app for mobile access
    5. GRANT - Require MDM compliant device for mobile access
    6. GRANT - Require managed device for Windows and Mac client app access
    7. BLOCK - Unsupported device platforms
...
.HOW-TO
    1. Connect to Azure AD via PowerShell to run this script use: Connect-AzureAD and then run .\Baseline-ConditionalAccessPolicies.ps1
...
New-AzureADMSConditionalAccessPolicy -DisplayName "GRANT - Require MAM approved app for mobile access" -State "Disabled" -Conditions $conditions -GrantControls $controls 

########################################################

## This policy enables MDM enforcement for iOS and Android devices (i.e. company-owned devices)
## MDM NOTES:
...
New-AzureADMSConditionalAccessPolicy -DisplayName "GRANT - Require MDM compliant device for mobile access" -State "Disabled" -Conditions $conditions -GrantControls $controls 
...
##    4. This policy blocks access from desktop client apps (e.g. Outlook, OneDrive, etc.)
...
New-AzureADMSConditionalAccessPolicy -DisplayName "GRANT - Require managed device for Windows or Mac client app access" -State "Disabled" -Conditions $conditions -GrantControls $controls 
